...
def create_dynamics(cfg: DynamicsCfg, device: str = "cpu"):
    ...
    if cfg.model_type == "quadrotor":
        ...
        drone_cfg = DroneConfig(
            ...
            min_body_rates=getattr(cfg, "min_body_rates", (-3.14, -3.14, -3.14)),
            max_body_rates=getattr(cfg, "max_body_rates", (3.14, 3.14, 3.14)),
            min_normed_thrust=getattr(cfg, "min_normed_thrust", 0.0),
            max_normed_thrust=getattr(cfg, "max_normed_thrust", 5.0),
            compensate_gravity=getattr(cfg, "compensate_gravity", False),
            torque_ratio=getattr(cfg, "torque_ratio", 1.0),
            thrust_ratio=getattr(cfg, "thrust_ratio", 1.0),
