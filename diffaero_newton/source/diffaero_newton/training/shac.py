"""SHAC-family algorithms for differentiable short-horizon training."""
...
class StateCritic(nn.Module):
    """Critic network that consumes privileged state instead of actor observation."""
...
class SHA2CAgent:
    """Short-horizon asymmetric actor-critic with obs actor and state critic."""
    ...
    def __init__(
        ...
        self.actor = Actor(obs_dim, action_dim, self.cfg.actor_hidden_dims, self.cfg.actor_log_std_init)
        self.critic = StateCritic(state_dim, self.cfg.critic_hidden_dims)
        self.target_critic = StateCritic(state_dim, self.cfg.critic_hidden_dims)
        self.target_critic.load_state_dict(self.critic.state_dict())
        for param in self.target_critic.parameters():
            param.requires_grad_(False)
...
class SHAC:
    ...
    def _collect_rollout(self, obs: torch.Tensor | Dict[str, torch.Tensor]):
        ...
        for step in range(self.cfg.rollout_horizon):
            ...
            next_obs, _next_state, loss_terms, reward, extras = self.env.step(action)
...
class SHA2C:
    """Training loop for short-horizon asymmetric actor-critic."""
    ...
    def __init__(
        ...
        state_dim = self._get_env_state().shape[-1]

        self.agent = SHA2CAgent(
            obs_dim=obs_dim,
            state_dim=state_dim,
            action_dim=action_dim,
            cfg=self.cfg,
    ...
    def _collect_rollout(self, obs: torch.Tensor | Dict[str, torch.Tensor]):
        ...
        for step in range(self.cfg.rollout_horizon):
            ...
            next_policy_obs = extras.get("next_obs_before_reset", self._policy_obs(next_obs))
            next_state_graph = extras.get("next_state_before_reset", next_state)
            terminated = extras["terminated"]
            truncated = extras["truncated"]
            reset = terminated | truncated
            self.last_extras = extras
        ...
        self.buffer.actor_loss_graph = cumulated_loss.mean() - self.cfg.entropy_coef * entropy_accum.mean()
        self.buffer.mean_entropy = entropy_accum.mean() / self.cfg.rollout_horizon
        final_state = self._get_env_state().detach()
    ...
    def _save_checkpoint(self):
        os.makedirs(self.cfg.save_dir, exist_ok=True)
        path = os.path.join(self.cfg.save_dir, f"sha2c_{self.iteration}.pt")
        self.agent.save(path)
        print(f"Saved checkpoint to {path}")
    ...
    def _get_env_state(self) -> torch.Tensor:
        if not hasattr(self.env, "drone"):
            raise RuntimeError("SHA2C requires an environment with privileged drone state.")
